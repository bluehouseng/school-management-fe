...
const Card = ({ name, roomsArr }) => {
  return (
    <div style={{ display: 'flex', background: '#ffff', width: "80%", margin: '2rem', borderRadius: '5%' }}>
      <div className="mt-8 ">
        <h2 className="text-xl leading-snug text-slate-800 font-bold mb-5">{name.blockName}</h2>
      </div>
      {
        roomsArr.map((item, idx) => {
          console.log(item)
          return (
            < CardData key={idx} Room_no={item.Room_no} status={item.Status} />
          )
        })
      }
    </div>
